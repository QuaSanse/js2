...
console.log(dataArr.url);
// Ключ авторизации - auth=5ADcB96BA48d3f80
// let auth = 'auth=5ADcB96BA48d3f80';

//полный адрес
// let url = new URL('http://getpost.itgid.info/index2.php');
...
function myFunction(data) {
    elemDom.innerHTML = data; // вывод на страницу
    console.log(data); // вывод в консоль
}

function sendGet(parAct, domElem) {
    let xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = checkStatus;

    xhttp.open('GET', `${dataArr.url}${dataArr.auth}${dataArr.action}${parAct}`, true);
    xhttp.send();

    elemDom = document.querySelector(domElem);
}

function sendPost(parAct, domElem) {
    let xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = checkStatus;

    xhttp.open('POST', `${dataArr.url}`, true);
    xhttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xhttp.send(`${dataArr.auth}${dataArr.action}${parAct}`);
...
function t12() {
    sendPost('3&num1=5&num2=7', '.out-12');
}
...
function t13() {
    sendPost('4&num1=1&num2=13', '.out-13');
}
...
function t15() {
    sendPost('6&num1=5&num2=15', '.out-15');
}
