...
function sendGet(domElem, parAct) {
    fetch(`${dataArr.url}${dataArr.auth}${dataArr.action}${parAct}`)
        .then(data => {
            try {
...
function sendPost(domElem, parAct) {
    fetch(dataArr.url, {
        ...
        body: `${dataArr.auth}${dataArr.action}${parAct}` // body data type must match "Content-Type" header
    })
        .then(response => response.text())
        .then(response => {
            console.log(response); // вывод в консоль
            document.querySelector(domElem).innerHTML = response; // вывод на страницу;
        })
}
...
function t4() {
    sendGet('.out-4', '4&num1=1&num2=20');
}
...
function t12() {
    sendPost('.out-12', '3&num1=5&num2=7');
}
...
function t13() {
    sendPost('.out-13', '4&num1=5&num2=13');
}
...
function t15() {
    sendPost('.out-15', '6&num1=8&num2=15');
}
